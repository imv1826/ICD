$(window).on('load', function () {
 ...
 $("#icd_thisticketisforme").closest("tr").hide();
    $.ajax({
        url: `/_api/icd_guests?$count=true&$filter=_icd_application_value eq ${storedAppID} and _icd_contact_value eq ${storedUserId} and statecode eq 0 &$select=icd_guestid`,
        type: "GET",
        cache: true,  
    ...
    if(numberOfAssignedTable >= storedTableSeat){
    ...
    console.log('Event listener attached to icd_conferencetype');
    } else {
